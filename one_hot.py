...
def train(X, z, saved_model_name):
    ...
    param_dist = {'objective': 'binary:logistic', 'n_estimators':20, 'n_jobs':7, 
                  'max_depth': 10}
...
with open("features/text_len.csv", encoding="utf-8") as csvfile:
...
with open("features/bothAreFamous.csv", encoding="utf-8") as csvfile:
    reader = csv.reader(csvfile)
...
with open("train_part/train.partaa", encoding="utf-8") as f:
    ...
    z = y[:,0].tolist()
    train(X,z,"model0_onehot")

    z = y[:,1].tolist()
    train(X,z,"model1_onehot")

    z = y[:,2].tolist()
    train(X,z,"model2_onehot")

    z = y[:,3].tolist()
    train(X,z,"model3_onehot")
