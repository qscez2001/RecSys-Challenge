...
with open("val.tsv", encoding="utf-8") as f:
    ...
    for line in f:
        ...
        for feature, idx in all_features_to_idx.items():
            ...
            # 'tweet_timestamp': 8,
            if idx == 10 or idx == 11 or idx == 15 or idx == 16 or idx == 8:
                new_X.append(features[idx])
...
with open('model0_', 'rb') as fd:
    clf = pickle.load(fd)

with open('model1_', 'rb') as fd:
    clf2 = pickle.load(fd)

with open('model2_', 'rb') as fd:
    clf3 = pickle.load(fd)

with open('model3_', 'rb') as fd:
    clf4 = pickle.load(fd)
